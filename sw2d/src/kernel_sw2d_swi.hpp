...
double sw2d_swi(queue &q, const std::vector<int> &wet, const std::vector<float> &eta,
  ...
  auto event_first = q.submit([&](handler &hnd) {
    ...
    hnd.single_task<class map49>([=]() [[intel::kernel_args_restrict]] {
#if FPGA || FPGA_EMULATOR
      [[intel::ivdep]]
#endif
  ...
  q.submit([&](handler &hnd) {
    ...
    hnd.single_task<class map55>([=]() [[intel::kernel_args_restrict]] {
#if FPGA || FPGA_EMULATOR
      [[intel::ivdep]]
#endif
      for (int global_id = 1; global_id < DOMAIN_SIZE; ++global_id) {
    ...
    hnd.single_task<class map75>([=]() [[intel::kernel_args_restrict]] {
      ...
      for (int global_id = 1; global_id < DOMAIN_SIZE; ++global_id) {
        ...
        etan[F2D2C((((NY + 1) - 0) + 1), 0, 0, j, k)] =
            eta[F2D2C((((NY + 1) - 0) + 1), 0, 0, j, k)] - dt * (hue___dyn - huw___dyn) / dx -
